...
def generate_luck_score(ti):
    luck_score = random.random()
    print(f"LUCK_SCORE generated: {luck_score}")


    if luck_score > 0.5:
        ti.xcom_push(key='luck_score_result', value='TASK_B')
    else:
        ti.xcom_push(key='luck_score_result', value='TASK_C')


def write_fan_letter_supe_a():
